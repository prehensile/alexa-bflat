...
var stateHandlers = {
    'LaunchRequest': function () {
        this.emit('PlayAPartIntent');
    },
    'PlayAPartIntent': function () {
        // play a part
...
var controller = function () {
    return {
        play: function (text) {
            ...
            var part = audioData.randomPart();

            text = "Section " + (part['index']+1);
            var partUrl = part['url'];

            this.response.speak(text).audioPlayerPlay('REPLACE_ALL', partUrl, partUrl, null, 0);
            this.emit(':responseReady');
        },
        stop: function (text) {
            /*
